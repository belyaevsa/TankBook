...
9. **Server never interprets domain records**; no domain queries/search/stats endpoints — such needs are client-side computations. (`docs/API.md`)
...
ios/                     # (planned) Xcode project — SwiftUI + GRDB (decided; see SCHEMA.md)
backend/                 # (planned) ASP.NET Core solution implementing API.md; Postgres; MinIO for local dev
```
