...
| `docs/CONFIG.md` | Remote config: the three precedence layers, `apiBaseUrl` guardrails and auto-revert, delivery by poll with push as a hint; the build channels (Debug / Beta / Release) and the `EXPERIMENTS` rule for beta-only features | Adding a remote flag, changing endpoints, anything that must ship without an App Store release; adding, promoting or removing a beta experiment |
...
| `docs/DEFECT-PATTERNS.md` | The eight shapes this codebase actually produces - sibling defects, silently-reachable fallbacks, docs naming behaviour with no call site, features built on an entity nothing creates - each with the check that catches it | **Before writing any brief or fix**, and when a defect looks familiar |
