...
pipeline over **photographs**. This folder exists because the corpus holds
**zero non-fuel receipts**: as of 2026-09-11 the 61 receipt images are all
fuel (or mixed fuel + one non-fuel line) and the 84 pump displays are fuel by
definition, so a vocabulary that reads the KIND of an expense has nothing real
to be measured against yet. These are hand-authored **OCR-text** fixtures, not
...
extractor produced measures nothing. Add a real photograph here (with a `.jpg`
and an OCR dump) when the product owner's next non-fuel receipt arrives, and
this folder becomes the same kind of corpus as `receipts/`.
