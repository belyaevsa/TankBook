...
## Batch 9 (2026-09-22, product owner): twelve clips - France x10, US, Québec

Same cut as Batch 5: only the seconds that show the display, no audio, no metadata, nobody in
frame (`video-031` and `video-033` were cut after the person left the frame). Reference quads
placed by the orchestrator from a 5 % grid, unreviewed; `pump_reader.track --videos` ran on each.

| video | frames | what | truth |
|---|---|---|---|
| `video-029-unknown-this-sale-dollars-gallons-running-display-us` | 320 (3840x2160, 24 fps, 13.3 s) | US stock clip (Pexels `12731020`): a white head, `This Sale` / `$` / `Gallons`, **counting up** 4.38 / 1.566 → 8.33 / 2.976; no price window on the face | price `2.799` constant (`1.654 x 2.799 = 4.63`, `2.976 x 2.799 = 8.33`); currency `USD`, the `liters` field holds **gallons** |
| `video-030-tatsuno-leclerc-static-display-4868-2653l-1835-fr` | 120 (576x1024, 30 fps, 4 s) | **Tatsuno** at an E.Leclerc (the MADIC terminal below), the owner's `IMG_33067.MOV` cut to 4-8 s: a **finished, static** fill 48.68 € / 26.53 L; the price line **alternates** between `1.835 €/L` and `E. 18 €/L` about once a second | `26.53 x 1.835 = 48.68`; reference `075.jpg` is an `E. 18` frame - the quad is the same |
| `video-031-wayne-breuillet-one-cent-litre-running-display-fr` | 345 (1080x1920, 30 fps, 11.5 s) | **Wayne** at Breuillet (Essonne), the viral **0,010 €/L** error (`LIVRAISON MINIMALE 5 LITRES`, `€uro / Litres / €uro/litre`): counting 0.99 / 99.05 → 1.01 / 101.4, a burned-in `Oumse dia` watermark and an ad screen above; cut 11-22.5 s of the Facebook clip `1276164207291216` | price `0.010` constant; the total is the litres / 100, so a one-cent step every 1 L (`99.95 x 0.010 = 1.00`) - the arithmetic label tolerates the truncation |
| `video-032-unknown-quebec-diesel-snow-dollars-running-display-ca` | 404 (1080x1920, 30 fps, 13.4 s) | Québec, **snow falling on the glass**, `$` / `Litres` / `$/Litre`, DIESEL, pump 5, `INSTRUCTIONS DE PROGRAMMATION` keypad: counting 45.01 / 20.845 → 62.22 / 28.9 - **three-decimal litres**; Facebook clip `34280867621559575` whole | price `2.149` constant (`24.672 x 2.149 = 53.02`); currency `CAD`; no price window annotated |
| `video-034-unknown-lcd-refresh-glitch-static-5011-2170l-2309-fr` | 138 (720x1280, 21.6 fps, 6.4 s) | a French face with a caption band, **static** 50.11 / 21.70 at 2.309 - and the LCD's multiplexed refresh caught by the phone's shutter: **segments drop out frame by frame** (a `5` loses its top, a `0` its left side, the price reads `2 09` for a frame), the "glitches" the owner sent it for; whole clip | `21.70 x 2.309 = 50.11`; 12 frames drop from tracking |
