...
| `video-007-adast-running-display-kz` | 212 (720x1280, 30 fps, 7.1 s) | ADAST, Kazakhstan (`АЗК` canopy sign), one LCD with three right-aligned rows (`СУММА` / `ЛИТРЫ` / `ЦЕНА/ЛИТР`), integer sum in tenge; shot from a car, the display is ~13 px per digit; whole clip kept | price `294` constant; ~6100 / 20.9 → ~7400 / 25.2 (`22.65 x 294 = 6659.1` shows `6659`, `24.09 x 294 = 7082.5` shows `7082`) |
...
| `video-009-adast-running-display-slow-fill-kz` | 300 (720x1280, 30 fps, 10 s) | ADAST, Kazakhstan (Kazakh caption `Шөлдеп қапты` burned in), same three-row LCD as `video-007`, camera close and steady, a **slow** fill - 0.36 L in 10 s; the pan to the driver after 10 s was cut | price `233` constant; 14935 / 64.10 → 15019 / 64.46 (`64.10 x 233 = 14935.3`, `64.36 x 233 = 14995.9` shows `14996`, `64.46 x 233 = 15019.2` close - this one rounds) |
