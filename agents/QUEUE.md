...
|---|---|---|---|---|
| `RV.253` | F10 | flash | (dispatching) | the quota card fed by the real 429 |

**IMPLEMENTED (14).** Scenario 8: `RV.155` `88bb4e2`, `RV.108` `d979e49`, `RV.143`, `PJ.58`, `RV.158+RV.138` shipped; `PJ.59` shipped (`4c774f42`) - scenario 8's build rows are closed. **Walked**: `J10` implemented (`1d6175a4`), `F9` implemented, `J2` NOT implemented - one gap, `RV.255` (the import selects nothing, so an existing-car user lands on the old car) - shipped. `RV.253` in flight, then the F10 walk and the J2 re-walk. `J11`/`J11a` cannot be walked yet: `RV.239`, `RV.249`, `RV.253`, `PJ.35` are open. Filed `RV.254` [v1.0.x] (the span-days log field RV.158 left). New from verification: `RV.253` (the quota card is fed only by a DEBUG fixture).
