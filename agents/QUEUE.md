...
|---|---|---|---|---|
| `RV.240`+`RV.268` | J9 | flash | (dispatching) | the anomaly card: no dismissal reason; the act reminder due next year |
...
**Single-row follow-ups, ordered (briefs written 2026-09-12):** `RV.241` shipped → `RV.263` (in flight) (F6a's one gap: a declared currency is not editable - hard rule 13) → `RV.227`+`RV.235` (pinned-bar padding, two screens) → `RV.229` → `RV.256` → `RV.257`+`RV.258` (test determinism) → `RV.245` → `RV.264`+`RV.265` (flag-surface copy, one dispatch) → `PJ.60` (owner: drop `Expense.recurrence`) → `RV.240`+`RV.268` (owner: no dismissal reason; the act reminder due next year) → `RV.234` (the unit-in-sentence seam) → `RV.248` (owner: reminder history surface - a screen, last).  `J1` implemented (walked 2026-09-12 after RV.241 + RV.251). `F6a` implemented (third walk, after RV.263). `F6` implemented (walked after RV.227 + RV.228). Then re-walks: F6b/F2 (after RV.229), J1 (after RV.241), J7c/J9/J7 as their owner decisions land.
