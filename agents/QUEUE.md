...
|---|---|---|---|---|
| `RV.284` | F10, F2, J11 | flash | (queued - provider down; server half first, it is what unblocks the owner's two rows) | a fill-up flagged consumption is rejected by the deployed registry forever and silently |
| `RV.285` | F4, J3 | flash | (queued - provider down) | the gateway's provider call has no bounded timeout |
| `CORPUS-2026-09-14` | J4, J3, J5, J7b | flash | (provider dead at the banner on four launches, 2026-09-14 morning - retry; the eight files sit untracked in the fixture folders until the run lands) | register the Gilbarco set, receipt-067, screenshot-009 and the Snabb parking invoice |
...
**Walks are the orchestrator's own from 2026-09-12** (product owner): no more `REVIEW-SCENARIO-*` / `REVIEW-JOURNEYS-*` pro dispatches; the briefs remain the method, the reports keep their names.

**Single-row follow-ups, ordered (briefs written 2026-09-12):** `RV.241` shipped → `RV.263` (in flight) (F6a's one gap: a declared currency is not editable - hard rule 13) → `RV.227`+`RV.235` (pinned-bar padding, two screens) → `RV.229` → `RV.256` → `RV.257`+`RV.258` (test determinism) → `RV.245` → `RV.264`+`RV.265` (flag-surface copy, one dispatch) → `PJ.60` (owner: drop `Expense.recurrence`) → `RV.240`+`RV.268` (owner: no dismissal reason; the act reminder due next year) → `RV.270` (the LPG-from-boilerplate fuel kind; was filed as RV.243 under a shipped id - F2 and J3 status lines cleared until it ships) → `RV.234` (the unit-in-sentence seam) → `RV.248` (owner: reminder history surface - a screen, last).  `J1` implemented (walked 2026-09-12 after RV.241 + RV.251). `F6a` implemented (third walk, after RV.263). `F6` implemented (walked after RV.227 + RV.228). Then re-walks: F6b/F2 (after RV.229), J1 (after RV.241), J7c/J9/J7 as their owner decisions land.
