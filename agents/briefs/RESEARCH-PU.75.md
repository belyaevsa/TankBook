...
*A run of `RESEARCH-TO-CODE.md` for PU.75. Product owner, 2026-09-23: **"review the published research to apply it into the code,
...
- **Row:** `PU.75` – Batched predictions, vectorised warps, and the Vision pass only when it decides
  (`docs/TASKS.md`)
- **Sources the row cites:** Apple's Core ML batch prediction (`MLModel.predictions(fromBatch:)` /
  `MLBatchProvider`) and Accelerate vImage (`vImageScale_*`, `vImageAffineWarp_*`) documentation;
  for the 31 MB detector: Howard et al., *Searching for MobileNetV3*, ICCV 2019, arXiv:1905.02244, and
  Jacob et al., *Quantization and Training of Neural Networks for Efficient Integer-Arithmetic-Only
  Inference*, CVPR 2018, arXiv:1712.05877; Core ML Tools' weight quantization/palettization docs.
- **The code seam it lands in:** `ios/Sources/TankbookCore/Extraction/PumpReader/PumpDisplayCapture.swift`
  (`decideAt` runs `textLineCount` before the fast verdict that ignores it), `PumpSegmentsModel.swift`
  (5 TTA crops per cell = 5 `prediction` calls), `PumpQuadWarp.swift` and `PumpReader.swift`
  (per-pixel Swift warps/resamples), `PumpPanelLocator.swift` (`downscaleRGB`), `PumpRowDetector.swift`.

Section 5: latency must be a RELEASE number (Debug is 7-25x slower and is not a latency fact); say how
to measure it on the Mac (`swift build -c release`, `pump-read` `timingsMs`) and on a device (PU.39's
Capture Lab). Every speed change must leave committed cells identical - name the check. For the
detector, say whether int8/palettized weights are expected to hold decision 10's tight-IoU gate.

## Where you may write

You are working in `/Users/sbelyaev/repos/fuel-counter-ios`. **READ-ONLY except one file:**
`agents/research/PU.75.md`. No code, no tests, no builds, no commits. Another agent or the
