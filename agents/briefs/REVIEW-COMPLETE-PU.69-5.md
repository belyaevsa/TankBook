...
Review 4 left one string: the closure sentence's "8.4 -> 2.7 ms" in the PU.69 row, now "8.4 -> 2.9-3.1 ms".
Verify that string, and that no "2.7 ms" for PU.69 remains in `docs/TASKS.md` or `docs/EXTRACTION.md`.

## Fourth pass (history)

Review 3 (`agents/reviews/PU.69-COMPLETENESS-3.md`) left two one-line residuals: the stale PU.70 comment in
`PumpRowDeskewCorpusTests.swift` (now row-free) and the latency span (now 8.4 -> 2.9-3.1 ms, every log named,
the 2.8 attributed to the no-refinement variant). Verify those two only; everything else was MET.
...
`agents/reviews/PU.69-COMPLETENESS-5.md`. No code, no builds that write, no commits. You may run
