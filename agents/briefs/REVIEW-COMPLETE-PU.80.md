...
- **Row:** `PU.80` – the corpus sweep ratchets, red on every intake (`docs/TASKS.md`; ticked `[x]` in
  the working tree pending THIS review - the commit waits on your verdict)
- **Research note:** none (no paper; a test-maintenance row). Item 1 therefore reads: the change does
  what the owner approved - *"we don't use PaddleOCR. It was too complicated and long to wait for it"* -
  and nothing more.
- **The diff under review** (working tree against HEAD; only these paths): 
  `ios/Tests/TankbookCoreTests/PostSweepCorpusAdditions.swift`, `ios/Tests/TankbookCoreTests/PaddleOCRTests.swift`,
  `.claude/skills/corpus-intake/SKILL.md`, `docs/EXTRACTION.md` (the P4.13 "Retired" paragraph), the
  PU.80 row in `docs/TASKS.md`.
- **The orchestrator's gate evidence:** `swift test --filter "CorpusABTests|PaddleOCRTests"` 15/15;
  mutation (the `pump-328` declaration removed) red, `/tmp/agentlogs/pu80-mutation.log`, restored;
  `swiftlint lint` 0 errors.

**Row-specific:** item 2 (app path) and item 3 (pump measurement) do not apply - nothing here runs in
the app or moves a pump number; say so and move on. Do check: that every one of the 57 declared images
really is absent from `Spike/ReceiptSpike/fixtures/vision-ab/llm-<class>.json` (a declared image that
WAS swept would hide nothing but is wrong), that nothing still depends on the removed PaddleOCR
forward check, and that the LLM arm's coverage test still bites (the mutation log).

## Where you may write

You are working in `/Users/sbelyaev/repos/fuel-counter-ios`. **READ-ONLY except one file:**
`agents/reviews/PU.80-COMPLETENESS.md`. No code, no builds that write, no commits. You may run
