...
- **Row:** `RV.305` – a column-laid invoice commits a line amount as its total (`docs/TASKS.md`,
  ticked in the tree pending THIS review). Not a pump row: read the checklist for the expense total
  finder (app path = `FuelExtractor` via `CapturePipeline`; measurement = the expense and receipt
  corpus suites).
- **Research note:** none (engineering row, owner's rule of 2026-09-24). Item 1 reads: is the guard the
  narrowest correct change, and does it alter any OTHER fixture's total (receipts, expenses, fiscal,
  screenshots)? Check `redundantValue`'s other callers and receipt-001 / receipt-038, the documented
  redundancy cases.
- **The diff under review:** `ios/Sources/TankbookCore/Extraction/FuelExtractorTotalFinder.swift`
  (`grandTotalRead`'s last fallback) and the RV.305 row. IGNORE the concurrent PU.78 edits
  (`PumpReadingLaw.swift`, pump tests, `docs/EXTRACTION.md`) - not under review.
- **The orchestrator's gate evidence:** `swift test --filter "RV277|RV56|Expense|TotalFinder|FuelExtractor|Corpus"`
  119/119; mutation red (`/tmp/agentlogs/rv305-mutation.log`); `swiftlint` 0 errors. The full gate
  runs with PU.78's.

## Second pass

Review 1 (`agents/reviews/RV.305-COMPLETENESS.md`) was INCOMPLETE only on RV.301's row owning the
residual; the row now carries it (label-ranking decision, checks, sibling seam). Verify that and
re-confirm the rest briefly.

## Where you may write

You are working in `/Users/sbelyaev/repos/fuel-counter-ios`. **READ-ONLY except one file:**
`agents/reviews/RV.305-COMPLETENESS-2.md`. No code, no builds that write, no commits. You may run
