...
| 2026-09-12 (A+B) | `ed06ecca` | 3 rows (`PJ.100`, `PJ.101`, `PJ.200`), all one seam - guest-Home parity, the RV.197/RV.251 family; `PJ.200` is a finding against reviewed J7d, whose status line was cleared. 0 ticked-but-untrue |
| 2026-09-13 (A-D, the orchestrator) | `c11f1099` | 28 rows since the 2026-09-12 walks; 0 ticked-but-untrue (7 re-checked by grep, every row mutation-gated at ship); 1 row (`PJ.300` - a service line item's home money is never backfilled, re-homed or read); the two A+B unsettled reachability items settled; J4 (owner photos) and J8b/J13 (`RV.181`) remain without a status line |
| 2026-09-19 (A-D, the orchestrator) | `787cda7a` | 10 rows since 2026-09-18 (the second 1.1 tranche); 0 ticked-but-untrue (all 10 re-checked by grep); **0 new rows**; two findings fixed in the walk - the station brand picker was a screen `SCREENMAP.md` did not list (now in the mermaid, the inventory and `ScreenRouteBindings`), and `StationBrandRegistry` decoded the bundled pack per call; `PR.37` (the nudge sender) was the one remainder and was filed at ship (`diagnostics/REVIEW-JOURNEYS-2026-09-19.md`) |
| 2026-09-19 second walk (A-D, the orchestrator) | `942406b1` | 7 rows since the morning walk (`RV.296`/`RV.297`, `RV.298`, `PR.37` cut, `PJ.44`, `PJ.40`, `RV.300`) - run early because `PJ.40` fired shapes 2 and 3 at once; 0 ticked-but-untrue (all 7 re-checked by grep); **0 new rows**; one doc drift fixed in the walk (the S5 card was in neither `SCREENMAP.md` root row); `PR.37`'s cut verified against the sender's call site (`diagnostics/REVIEW-JOURNEYS-2026-09-19b.md`) |
| 2026-09-23 (J4, F2 and the re-check, the orchestrator) | `07f4f319` | 320 commits since 2026-09-19, almost all the pump-reader tranche and annotator tooling; 0 ticked-but-untrue (RV.303, RV.288, RV.292 re-checked by grep); **1 row** (`PJ.500` - decision 11's F2 confirm for a disagreeing shown price has no reader in the app); J4's reader paragraph and decision 11's promise corrected in the walk (`diagnostics/REVIEW-JOURNEYS-2026-09-23.md`) |
| 2026-09-18 (A-D, the orchestrator) | `66576113` | 17 rows since 2026-09-13 (the 1.1 tranche among them); 0 ticked-but-untrue (6 re-checked by grep); **0 new rows** - the walk's findings were all filed during the tranche (`RV.296`-`RV.299`) or already rows (`RV.115`); two doc drifts fixed in the walk (`SCREENMAP.md` had no edge for the share-sheet entry; `SYNC.md` named a WiFi-change prefetch trigger that does not exist); `Station.brand` has no production writer - PJ.31 is per-station until `RV.115` (`diagnostics/REVIEW-JOURNEYS-2026-09-18.md`) |
