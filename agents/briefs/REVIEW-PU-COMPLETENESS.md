...
- **Research note:** `agents/research/<PU.nn>.md` (if the row cites papers)
- **The diff under review:** `git diff` of the working tree against `<commit>`, paths `<list>`
- **The orchestrator's gate evidence:** `<log paths with exit codes and counts>`

## Where you may write

You are working in `/Users/sbelyaev/repos/fuel-counter-ios`. **READ-ONLY except one file:**
`agents/reviews/<PU.nn>-COMPLETENESS.md`. No code, no builds that write, no commits. You may run
