# REVIEW-SCENARIO run: F2 - 2026-09-12 (first walk)

- **Scenario:** `F2` - Scan recognized wrong data - the most dangerous failure (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-F2-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-F2-2026-09-12.md`
- **Context:** Rows closed: `RV.243` (the photo survives a deferred read; `23ec0d85`) and `RV.229` (`7912e19a`) - the import path's consumption outlier is named as one. Earlier: `RV.218` (CHECK 5 on the edit screen), `RV.211`/`RV.164`. The spine is hard rule 13 and rule 4: a wrong read is a default the user corrects, the receipt total never becomes the fuel amount on a mixed receipt, and every surface that can carry a misread names the field to check. Walk capture -> confirm -> save -> edit -> the flagged/excluded lists for a misread litres, a misread odometer and a misread total. Deferred rows are N/A for the v1 verdict.

**A build agent (RV.256) and one other review are working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write; the other review may be writing a different heading.**
