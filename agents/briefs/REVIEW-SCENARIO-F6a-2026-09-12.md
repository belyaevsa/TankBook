...
- **Report path:** `diagnostics/REVIEW-SCENARIO-F6a-2026-09-12.md`
- **What changed since the first walk** (`diagnostics/REVIEW-SCENARIO-F6a-2026-09-11c.md`): `RV.228` shipped (`0ff50af0`) - the `units` question is declared reserved (not v1) in every text, F6a bullets 1 and 4 now say units is N/A for v1, the preview hint names the date format or currency. Read the first report and re-check ONLY what it gated on. Deferred rows (`PJ.44` [v1.1]) are N/A. `RV.227` is open but names F6, not F6a - cite it if it touches F6a's promises, do not re-file.

**A build agent (RV.241) is working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write.**
