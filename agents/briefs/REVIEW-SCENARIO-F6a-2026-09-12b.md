...
- **Scenario:** `F6a` - The import preview: nothing is written until the user says so (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-F6a-2026-09-12b
- **Report path:** `diagnostics/REVIEW-SCENARIO-F6a-2026-09-12b.md`
- **What changed since the second walk** (`diagnostics/REVIEW-SCENARIO-F6a-2026-09-12.md`): `RV.263` shipped (`6355cf83`) - the currency card renders for any `currency` ambiguity, pre-filled with the declared code; `needsCurrencyAnswer` gates the commit only for a no-column file. Re-check ONLY what the second walk gated on (its one proposed row). Deferred rows are N/A; `RV.227` is open under F6 (the source picker), cite it if it touches F6a.

**A build agent (RV.227+RV.235) is working in this checkout - on the import SOURCE picker among others. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write.**
