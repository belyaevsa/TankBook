# REVIEW-SCENARIO run: F6b - 2026-09-12 (first walk)

- **Scenario:** `F6b` - A flagged import row is fields, not a line of CSV (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-F6b-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-F6b-2026-09-12.md`
- **Context:** Rows closed: `RV.229` (`7912e19a`) - a consumption outlier gets its own kind, label and next step on the review card; `RV.263` (declared currency editable), `RV.228`. Open and cited, not re-filed: `RV.264` (the review header composes its count - polish; decide whether it touches F6b's promise). Walk every flag kind the review can carry to its card: the fields, the sentence, the next step, the two exits (import as-is, leave out), and the source line. Deferred rows are N/A for the v1 verdict.
