...
- **Scenario:** `F7` - Restore fails or comes back empty (J11's nightmare) (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-F7-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-F7-2026-09-12.md`
- **Context:** Rows closed today: `RV.239` (the import rows on `EmptyRestoreView` and `RestoreUnreachableView` were dead `NavigationLink(value:)`s inside the sign-in sheet - now they reach the wizard; `2f10dad1`). Earlier: `PJ.3` (restore intent carried by the door tapped), `PJ.13`, `RV.197`. Deferred and N/A: `PJ.39` [v1.1] (interrupted-restore row) and the other deferred F7 row - cite them. The spine is hard rule 8 and rule 7 on the screen that exists to prevent data loss: every one of F7's three sources (wrong provider, empty account, backend unreachable) must name its next step and survive being ignored, and a guest's local data must never be touched by a failed restore. Walk each source to the screen the user sees and the door it offers.

**A build agent may be working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write; another review (J11) may be writing a different heading.**
