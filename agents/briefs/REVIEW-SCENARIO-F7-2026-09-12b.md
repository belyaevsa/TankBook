...
- **Scenario:** `F7` - Restore fails or comes back empty (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-F7-2026-09-12b
- **Report path:** `diagnostics/REVIEW-SCENARIO-F7-2026-09-12b.md`
- **What changed since the first walk:** `RV.259` shipped (`1cf79ac8`) - a second empty account after a provider switch lands on the empty-restore screen, never a second wrong-provider question. Read `diagnostics/REVIEW-SCENARIO-F7-2026-09-12.md` first and re-check ONLY what it gated on. **One thing the first walk did not see and the J11 walk did**: `RV.260` [!] is open - the "Import a file you exported yourself" row opens the third-party importer, which cannot read a Tankbook backup. It names F7 source 3. Decide whether F7's promise survives with that row open (it is being built now) and say so; do not re-file it. Deferred rows are N/A.

**A build agent (RV.260) is working in this checkout on the restore-failure screens. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write.**
