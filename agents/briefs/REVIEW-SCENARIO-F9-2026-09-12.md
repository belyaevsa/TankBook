# REVIEW-SCENARIO run: F9 - 2026-09-12 (first walk)

- **Scenario:** `F9` - Currency rate unavailable for that date (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-F9-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-F9-2026-09-12.md`
- **Context:** Rows closed today: `RV.158` (the drain stops at the server's stated `coverageFloor`, paces, dedupes; `38b09792`) with `RV.138` (carry-forward horizon, server). Earlier: `RV.135` (historical feed), `RV.139`, `RV.140`, `RV.152`. Open but deferred: `RV.254` [v1.0.x] (span-days log field). The owner's own log has rate-pending rows dated in ranges the server answers with nothing - check what the F9 footnote promises against what `SCHEMA.md` -> Exchange rates now says coverage is, and whether the user is TOLD when a date is below the floor rather than left pending forever. Deferred rows (`[v1.0.x]`/`[v1.1]`/`[v1.x]`/`[v2]`) are N/A for the v1 verdict - cite them, do not re-file them.

**A build agent (PJ.59) and two other scenario reviews are working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write; another review may be writing a different heading.**
