# REVIEW-SCENARIO run: J10 - 2026-09-12 (first walk)

- **Scenario:** `J10` - Cross-border trip (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-J10-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-J10-2026-09-12.md`
- **Context:** Rows closed today: `RV.143` (a home currency arriving by sync re-homes on the receiving device; `268ac4ca`). Earlier: `RV.140` (re-home on the originating device), `RV.136`, `RV.144`. The story is a foreign fill-up with its own currency and a home figure at the entry date's rate - walk the capture, the manual door, the confirm sheet's currency chip, the Trends/Home totals, and both sync directions. Deferred rows (`[v1.0.x]`/`[v1.1]`/`[v1.x]`/`[v2]`) are N/A for the v1 verdict - cite them, do not re-file them.
