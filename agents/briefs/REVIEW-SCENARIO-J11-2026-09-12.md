...
- **Scenario:** `J11` - New phone / platform switch (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-J11-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-J11-2026-09-12.md`
- **Context:** Rows closed today: `RV.239` (the import rows on both restore-failure screens reach the wizard; `2f10dad1`), `RV.249` (cursor keyed by account, monotonic; `a7d59566`), `RV.253` (`7b234946`), `RV.155` (`88bb4e2`), `RV.108` (`d979e49`). `J11a` was walked today and is implemented (`diagnostics/REVIEW-SCENARIO-J11a-2026-09-12.md`) - do not re-walk sign-in itself, cite it. Open and not deferred, cited not re-filed: `RV.256` (persisted sync state one key - display after a switch). Deferred and N/A: `PJ.35` [v1.1] (photo prefetch), `PJ.39` [v1.1]. The spine: a fresh install + sign-in pulls the whole account from 0 (`SYNC.md:150`) and that IS restore - walk the pull of 1,600 records with attachments (the owner's real account), what the user sees while it runs, what happens if it is interrupted, and how the old phone is retired (device list, revoke, `410`).

**A build agent may be working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write; another review (F7) may be writing a different heading.**
