...
- **Scenario:** `J11` (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-J11-2026-09-12b
- **Report path:** `diagnostics/REVIEW-SCENARIO-J11-2026-09-12b.md`
- **What changed since the last walk:** `RV.260` shipped (`d99824fb`) - the local restore-from-backup door: `Route.restoreFromBackup` takes the exported folder with no network and no session, reachable from both restore-failure screens (the backup door first, the third-party importer beside it) and from Settings beside Export. `RV.261` shipped (`24a7683a`) - the Restoring screen renders the last odometer's recency and J11's source-device clause is marked [v2]. Read `diagnostics/REVIEW-SCENARIO-J11-2026-09-12.md` first and re-check ONLY what it gated on. Open and not deferred, cited not re-filed: `RV.256` (persisted sync state one key - display after an account switch), `RV.257` (a nondeterministic sign-in L4 - test determinism, not a promise). Decide whether either touches this scenario's promises and say so. Deferred rows are N/A.

**No build agent is running. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write; the other re-walk may be writing a different heading.**
