...
- **Scenario:** `J11a` - First sign-in (there is no "registration") (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-J11a-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-J11a-2026-09-12.md`
- **Context:** Rows closed today: `RV.249` (the pull cursor is keyed by account and monotonic, migrated once; `a7d59566`), `RV.155` (the cursor advance is durable at the pull that earned it; `88bb4e2`), `RV.253` (`7b234946`). Earlier: `PJ.13` (the just-signed-in card), `RV.197`. Open and NOT deferred, cited not re-filed: `RV.256` (persisted sync state is one account-unkeyed key - a display defect after an account switch, decide whether it touches J11a's promises). Deferred rows are N/A for the v1 verdict. The story's spine: sign-in IS registration (no separate step), the first pull from 0 IS restore, and a guest's local data survives the sign-in - walk the guest-with-data case as well as the fresh-install one, and every leaked-session trap `docs/DEFECT-PATTERNS.md` names.

**A build agent (RV.239) is working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write.**
