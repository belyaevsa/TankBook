...
- **Scenario:** `J2` - Switching from another app (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-J2-2026-09-12
- **Report path:** `diagnostics/REVIEW-SCENARIO-J2-2026-09-12.md`
- **Context:** Rows closed today: `PJ.58` (imported service line items home in the car's currency; `ed2d401b`). Earlier: `RV.185`+`RV.187` (an imported car keeps its declared currency and gets a name; every entry says what it is), `RV.176`, `PR.28`, the MFM parser rows. Deferred: `PJ.21`. The owner's own multi-year MFM import is the reference case - six of the ten defects they reported lived here. Walk the import from file pick to the first Home render with the imported car selected, in a non-EUR currency. Deferred rows (`[v1.0.x]`/`[v1.1]`/`[v1.x]`/`[v2]`) are N/A for the v1 verdict - cite them, do not re-file them.
