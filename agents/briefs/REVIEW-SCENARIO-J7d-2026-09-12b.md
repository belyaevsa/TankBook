...
- **Scenario:** `J7d` - A reminder is born [v1.1] (`docs/JOURNEYS.md`)
- **Run id:** REVIEW-SCENARIO-J7d-2026-09-12b
- **Report path:** `diagnostics/REVIEW-SCENARIO-J7d-2026-09-12b.md`
- **What changed since the first walk:** its status line was cleared when the 2026-09-12 journeys walk found the permanent Home row absent for a GUEST (`PJ.200`); `PJ.200` shipped with `RV.251` (`e6972b3d`) - `HomeRemindersEntryRow` is one shared view both layouts render. Read `diagnostics/REVIEW-SCENARIO-J7d-2026-09-12.md` first and re-check ONLY the guest path of "Plan it" and "Discover it", plus anything the first walk gated on. Replace the cleared-status note under the heading with the status line on IMPLEMENTED. Deferred rows are N/A.

**A build agent (RV.228) is working in this checkout. Write ONLY your report and, on IMPLEMENTED, this scenario's status line - re-read `docs/JOURNEYS.md` immediately before that one write.**
