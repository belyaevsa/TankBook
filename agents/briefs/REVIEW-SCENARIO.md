# REVIEW-SCENARIO - the recurring completion review for one user story

*The template. Every run gets its own dated copy narrowing this to one scenario
(`agents/briefs/REVIEW-SCENARIO-<id>-<date>.md`), so the run history stays honest about which
stories have actually been walked.*

**Dispatch this when `python3 scripts/scenario-index.py` lists a scenario under READY FOR REVIEW** -
every task row naming that journey is closed. **A scenario is not implemented because its tasks are
...
- **Scenario:** `<id>` - `<title>` (`docs/JOURNEYS.md`)
- **Rows that named it:** `<the closed rows, from scenario-index.py>`
- **Adjacent scenarios it hands off to or from:** `<ids>`
