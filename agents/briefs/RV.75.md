...
**The merged list's EMPTY state belongs to `RV.76`**, which replaces the dashed card with a filled
primary action (`design/screens/RemindersEmpty.dc.html`). Render what the per-car screen renders
today and leave the weight of that action alone, or the two tasks fight over one view.
