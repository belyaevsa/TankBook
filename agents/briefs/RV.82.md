# RV.82 - RemindersDeepLinkUITests must pass alone

## The defect

Run `RemindersDeepLinkUITests` by itself against a device with no Keychain session and `waitForCar`
fails: Home renders the **guest** layout, which has no `carSwitcherButton`. It passes when a session
is planted (`-seedSettingsSynced`) or when a sign-in suite ran earlier **in the same simulator
session**, because **the Keychain survives between test runs** - `simctl uninstall` does not clear
it (learned 2026-08-31).

**This is worse than a flake because it is deterministic in both directions**: green in a full run,
red alone. So the full suite certifies it, and anyone re-running the one suite to check a fix sees a
failure that has nothing to do with their change. This is the third instance of the order-dependence
family (2026-08-30: suites that only pass in company; 2026-08-31: the Keychain surviving an
uninstall), which makes it a pattern rather than an accident.

## What to build

**Make the suite state its own preconditions** - plant the session the deep link needs in the
suite's own launch arguments, rather than inheriting whatever an earlier suite left behind.

**Do not** make the test tolerate the guest layout. That asserts the bug.

**Audit the siblings** that also touch session state - `RemindersNotificationActionUITests`,
`SettingsUITests`, `AccountDevicesUITests` - and report which are self-sufficient and which inherit.
**Report differentiated; do not fix all of them silently.**

## Explicitly out of scope

The deep link's behaviour (RV.74, shipped and verified). Any production code change - if you find
yourself editing `ios/App/Sources`, stop and report why.

## Tests

**The proof is two runs, and both must be in the report with their exact commands:**
1. the suite **alone**, on a device whose Keychain was deliberately cleared;
2. the suite immediately **after** `SignInUITests` in the same run.

Both green. A suite that only passes in one of those orders is not fixed.

### Vacuous traps, named
- Running only the full suite - it passes today.
- Planting the session in a shared `setUp` that other suites also mutate: that moves the coupling
  rather than removing it.
- Asserting the deep-link route was set rather than the screen it lands on.

### Mutation
Remove the planted session from the suite's own launch arguments -> the alone-run must fail. If it
still passes, the Keychain was dirty and your clean-device step is not clean - say so.

## Docs to reconcile

`docs/TESTING.md` - if the clean-Keychain step is not already written down as a precondition for
running a session-touching suite alone, add it. That is the reusable half of this row.
## Where you may write
