...
/// APNs must stop riding the default, and a dead cloud-vision provider must stop
/// holding a request and a quota unit for the default (RV.285). The test reads
/// the actual clients the factory builds, so it asserts the wired numbers - not
/// a constant next to a different constant.
/// </summary>
public class HttpClientTimeoutTests
{
    ...
    public void NamedClientsCarryTheConfiguredTimeouts()
    {
        ...
        Assert.Equal(HttpClientTimeouts.Llm, httpFactory.CreateClient("llm").Timeout);
    }
}
