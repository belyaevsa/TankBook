...
- **Export shape, measured from a real file 2026-09-07** (anonymised copy at `Spike/ImportFixtures/drivvo/`, committed and validated): ONE file with three `##`-marked sections (`##Refuelling`, `##Expense`, `##Service`), **localised column headers** (the sample is Russian), **no currency column at all**, no vehicle column (one car per file), and mixed decimal conventions in the same row (`37.52` volume beside `6,414 л/100км`). The importer decisions that follow are recorded in RV.113.
