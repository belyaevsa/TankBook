...
- App icon (**settled 2026-08-30, product owner**): a **gas pistol** (`taillight`) and a **charging plug** (`headlight`) **facing each other** inside a thin `ink` frame, on `midnight`. The two objects are **generated artwork, not hand-drawn vectors** – produced with `imagegen`, extracted to an alpha layer with token colours, and composed; `design/brand/README.md` is the authority for the files and the render steps, `design/brand/icon-objects.png` is the master. Powertrain-agnostic at a glance, legible at 29pt, no text. The hand-drawn directions tried first (a merged "one grip" mark, then flat vector pistol-and-plug) were rejected as hard to read or clip-art-like; they sit on the canvas's Brand page (`design/screens/Icon.dc.html`) for the record. **Shipped 2026-08-30** as an iOS 18 single-size set in `ios/App/Resources/Assets.xcassets/AppIcon.appiconset` (the site's favicon, touch icon and `icon.svg` wrapper render from the same dark master): **Any/light** = `taillight.light` on `midnight.light`, **Dark** = `taillight` on `midnight`, **Tinted** = white glyph on transparent for the system tint. Full-bleed square – iOS applies the mask; never pre-round the corners.
