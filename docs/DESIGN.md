...
- App icon (**redrawn 2026-08-30, product owner**): a **gas pistol** (`taillight`) and a **charging plug** (`headlight`) **facing each other** – prongs toward the spout – inside a thin `ink` frame, on `midnight`. Two unmistakable objects, nothing merged, so each reads on its own at 29pt; powertrain-agnostic at a glance; no text. The directions considered (a merged "one grip" mark was tried first and rejected as hard to read) sit on the canvas's Brand page (`design/screens/Icon.dc.html`). **Shipped 2026-08-30** as an iOS 18 single-size set in `ios/App/Resources/Assets.xcassets/AppIcon.appiconset`, rendered from `site/static/icon.svg` (the one source of the mark): **Any/light** = `taillight.light` on `midnight.light`, **Dark** = `taillight` on `midnight`, **Tinted** = white glyph on transparent for the system tint. Full-bleed square – iOS applies the mask; never pre-round the corners.
