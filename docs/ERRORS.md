...
| Device revoked (410) | Card: "This device was signed out – sign in to reconnect. Your data on this phone is untouched." | Sign in · stay local |
| Session expired (refresh token rejected - PR.1) | Card: "Your session has expired – sign in again. Your data on this phone is untouched." The access token was refreshed and the refresh token came back rejected, so the session is gone - an auth event, **never "update the app"** (hard rule 7). The queue stays dirty (S7), nothing is lost | Sign in · stay local |
