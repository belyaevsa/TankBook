...
| Pull interrupted (network drop mid-restore) | "Connection dropped – restore continues when you're back online." with a "Restored so far" card (cars, entries) when a page landed - `RestoreOutcome.interrupted(stats?)`, rendered by `RestoreInterruptedView` (PJ.39). Entries already pulled remain usable; the persisted cursor makes every later pull resume. Distinct from the server-down row below: an `offline` transport is the user's connection, never the service | Open my garage (partial, keeps filling) - absent when nothing landed · Retry now · Not my account / Sign out |
...
| **Entry the server rejected structurally (a 422, RV.284)** | A `icloud.slash` badge on the entry (amber, distinct from the conflict chevron) - the same attention family, tapping through to Edit entry. The accessibility label is the next step: "Could not sync this entry – update the app or edit it to retry" | **Edit the entry (it re-pushes) or update the app.** A 422 is not unavailability - the same bytes are rejected forever - so the honest next step is a newer app or a newer server, never the user's data (hard rule 7). The entry stays on this phone throughout |
...
| Archived car returned via sync (S5) | Quiet notice card on Home and in the Garage, read from the device-local `vehicleReturn` row the resurrect writes: "Saab 9-3 came back with 1 new entry – stays archived." - a full plural phrase per count, never composed. Both answers are real and consume the notice on both surfaces | Delete again (the car and its rows go back to the tombstone, dirty, in Recently deleted for the undo window) · Keep (the car stays archived with its entries) |
| Post-outage sync batch (S7) | Toast "Synced. 2 entries need a look" | Tap → Log filtered to flagged entries · ignore (badges remain) |
| Reminder due (RV.122, 2026-09-19) | The **reminder chip strip** across the top of Home: one amber chip per attention-due reminder of the selected car, in due order - the title, and under it what is coming and when or how far in the reminder's own terms ("in 12 days", "in 420 km", "Due at 118 920 km" when the odometer is unknown) - and, always last, the **All reminders** chip, a door that looks like a door. An overdue chip stays amber and says so in words ("Overdue · 3 days ago", "50 km overdue") - never a red chip (hard rule 5). **Nothing due: the strip is absent**, not empty - the calm Reminders row below is the door then | Tap a chip → Reminders (the merged list, landing on that reminder with its completion sheet surfaced) · tap **All reminders** → Reminders (the merged list) |
| Consumption drift detected (J9) | Amber insight card in the Log, in the car's own unit, naming BOTH windows compared ("Consumption is up 21% vs a year ago" + "Last 90 days: 6.5 L/100km · a year earlier: 5.4 L/100km"); tap → evidence (chart of the drift + what it costs per month at the driver's own recent prices – RV.121, never a guessed cause, docs/VISION.md → "What we will not tell a driver") | Act → creates a service reminder due next year, opened for edit · dismiss (one tap, suppresses the cause) – both always present (a card with only dismiss teaches nothing; only act is a nag) |
...
| **Pump display shows a different price (PJ.500)** – the displayed unit price is near but not equal to total ÷ litres (a loyalty discount, or a misread digit) | `warn`: "The display shows %1$@ a litre, but these numbers work out to %2$@ – a discount, or a misread? Check the total and the litres." The price field stays empty so the derived price is the entry's | Check total and litres · save (a real discount needs nothing more) |
...
| Odometer breaks timeline (F9a) | Amber + conflicting entry quoted: "Aug 17 already recorded 119 486 km." Receipt date pre-trusted. The quote is one full localised sentence per distance unit, chosen from the vehicle's own units (km or mi, EN and RU) – never a unit label spliced onto a shared stem (RV.126) | **The validator's ranked resolution list, rendered in its order** (PJ.34): fix odometer (preselected) · fix date (the validator ranks it only when the order check flags; when a printed receipt/QR date is ground truth it is marked as needing explicit override, and the confirmation names the evidence – "The receipt says 17 Aug – change the date anyway?", one full localised phrase per language) · save anyway (flagged). The ranking is a default, every fix stays editable (hard rule 13) |
| Odometer breaks timeline – pace (F9a) | Amber: "Odometer breaks the timeline – check it." (the pace check flags without a conflicting entry to quote) | Fix odometer (preselected) · save anyway (flagged). A pace-only flag has no order conflict, so the validator ranks no date fix – the reading, not the date, is the field to question |
| **The timeline neighbourhood (RV.117b, [v1.1])** – shown on Edit entry's F9a while the entry flags, reached both from Home's conflict badge and from the flagged list (the whole-row tap already opens Edit entry) | A card under the odometer: the surrounding entries charted with the offending point off the trend (amber diamond vs the neighbours' ink circles - shape and label as well as colour, docs/DESIGN.md), **every plotted point labelled with its odometer and date**, the bracketing rows (**previous entry, this one and the next entry when one exists**, DIN figures - no blank row when there is no next), and the bidirectional statement built from `EntryValidation.validRange` - **the view reads the validator's intervals, it never recomputes one**. Three copy cases per side, each a full localised sentence: **between** ("On 13 Jul, the odometer must be between 490 501 and 492 000 km"), **open end** ("If the odometer is 100 900 km, the date must be no earlier than 18 Jul" - a `nil` bound is never a sentinel), and **`.none` on the date side** - the row's point: when the reading sits at or below its previous one, no date between the neighbours works, and the sentence says so: "no date between the neighbouring entries works – check the odometer" (the odometer is the field to question). A `.none` on BOTH sides (the neighbourhood itself is inconsistent - a multi-year import) renders **one sentence per failed comparison, naming the offending PAIR** (the previous or the next entry, with its date and odometer, and the pace when CHECK 2 fired) - never a generic "the entries around this one", and never only the first culprit: an empty range with two disagreeing neighbours names both (RV.188). An entry with no odometer has no `validRange` and renders **no panel and no empty box** | Fix odometer (preselected) · fix date (needs explicit override) · save anyway (flagged) - the ranked fixes and Accept (RV.104) are unchanged; the panel is what makes them a last resort rather than the only option |
...
| A scanned fill-up whose slip also logged accepted expenses (a grouped save) could not keep the photo (RV.173) | **The whole group saves unattached and is reported ONCE.** The fill-up and every accepted expense reference no attachment at all – never the plan's intended id, which would point at an `Attachment` the failed write never wrote (the dangling id RV.149 left on the expense siblings). The group's `purchaseGroupId` survives, so the Log still shows one grouped moment; only the shared photo is gone. The one report is the SAME sentence and the SAME call as the single fill-up row above – never once per expense, so a three-line slip does not shout three times | Free up space and re-scan the receipt (the photo is gone from this save only; the whole group stands) |
...
> **A grouped save is all-or-nothing (2026-09-11, RV.173).** When the slip that failed to keep its photo also logged accepted expenses, the binding is the whole group's: the fill-up and every accepted expense reference the SAME effective attachment list – the write's outcome, not the plan's intended id – so a `.lost` write leaves all of them unattached and never one of them dangling. The group's `purchaseGroupId` is untouched, and the single report above still fires once for the save, never once per row. The shape-only line `receipt.group.photoLost` (`expenseCount`) records that a grouped save dropped its binding, with no filename, amount or station (hard rule 12).
...
| A service with no vendor and no line item (RV.214) | Save is disabled and the hint under it names the missing step: "Add a vendor or a line item to save" (RU "Добавьте поставщика или позицию, чтобы сохранить"). A vendor alone, or a single line (titled or categorised), is a record - the Log row names it (RV.187) - so the old demand for a TITLED line is gone. The create and edit doors call the same rule, so a wholly blank service is refused on both | Type a vendor · add a line item · scan the invoice. The edit door shows the same sentence when the same rule refuses |
| Multi-page scan interrupted | "Page 2 didn't save – rescan it or continue with 1 page." | Rescan page · continue |
| A km lifetime with no odometer (RV.212) | The odometer card warns amber: "Odometer is needed to schedule the next service." with **Fill** one tap away – but the save is **not** blocked. The entry saves (the lifetime is the user's fact and the app cannot use it yet) and the post-save offer sheet names the missing odometer: "A km interval needs an odometer on the record – use months, or edit the record.", with Create disabled | Fill the odometer (pre-filled value one tap away) · use months · remove the km lifetime · save without a reminder. The create and edit doors accept the same (lifetime, odometer) pair; only a mounted tire set still refuses (its mileage span anchors on the odometer, docs/SCHEMA.md) |
...
| Expense-mode scan's cloud reading in flight / refused (PJ.29; F4; RV.57, RV.65) | The expense form carries the SAME two surfaces the fill-up Confirm sheet does, with the same copy: while `/extract` is in flight, the dismissable `inkSoft` proceed note "A more reliable reading may still arrive. You can proceed now."; when the reading ends in a 401 the refresh could not fix, the `warn` "sign in again to use cloud reading" card. Both block nothing – Save stays reachable – and the local read already stands (hard rules 1, 7, 15). Under `.required` the update notice replaces the request, exactly as on Confirm | Proceed now (the note is a hint, not a wait) · sign in again from the account card · save the expense either way. A late cloud answer is never applied to the open form – it lands in the inbox |
| Invoice scan's cloud reading in flight / refused (PJ.29a; F4; RV.57, RV.65) | The service form carries the SAME two surfaces the fill-up Confirm sheet does, with the same copy: while `/extract` is in flight, the dismissable `inkSoft` proceed note "A more reliable reading may still arrive. You can proceed now."; when the reading ends in a 401 the refresh could not fix, the `warn` "sign in again to use cloud reading" card. Both block nothing – Save stays reachable – and the local split already stands (hard rules 1, 7, 15). Under `.required` the update notice replaces the request, exactly as on Confirm | Proceed now (the note is a hint, not a wait) · sign in again from the account card · save the service either way. A late cloud answer is never applied to the open form – it lands in the inbox |
| **[v1.1]** Invoice scan's cloud reading disagrees with a line of the local split (PJ.303; J7 "every page reaches the cloud") | The offer renders ON THE ROW it pairs with (by amount, then title - `LineMatcher` - never by position): the caption "From the invoice" over the cloud's line, dimmed (a suggestion, not a fact), with **Take** and **Keep** under it. A cloud line no local line matches renders as a dimmed "Also on the invoice" card below the split with **Add** / **Dismiss**. Nothing is applied on its own: an offer nobody answers changes nothing at save, the row is the user's as typed (hard rule 13, "keep mine" is the default). A late reading lists the same pairs on the inbox card (below) | Take (the row's title, category and cost become the invoice's) · Keep · Add the new line · Dismiss it · save with none of them answered - the local split persists unchanged |
| **[v1.1]** Invoice scan's cloud lines do not add up to the total it read (PJ.303; hard rule 4's arithmetic) | An amber `warn` line under the header total: "The reading doesn't add up to the invoice total – check the lines." (RU "Распознанные позиции не сходятся с итогом счёта – проверьте позиции."). The lines are still offered, each still take-or-keep; none is applied by the flag. Attention, not an error (hard rule 5): Save is untouched | Check the lines against the invoice · take the ones that are right · keep the rest · save either way |
| **[v1.1]** Invoice has more pages than the cloud reading takes (PJ.303; `extract.maxInvoicePages`, `docs/API.md`) | No cloud reading is started - a partial call would fail the arithmetic gate on every such invoice - and an `inkSoft` note names the cap: "This invoice has more pages than the cloud reading takes (6) – every page is kept and split on the device. Check the lines." Every page IS kept and split on the device (hard rule 8); the split stands (hard rules 1, 15). A page the camera lost is the separate "Multi-page scan interrupted" row above (PJ.43) - this row is about a cap, not a loss | Check the lines · edit the split · save. Nothing to rescan |
...
| A foreign expense/service is captured (RV.279) | The capture form has no conversion card - the currency chip row states the pick and the save takes the entry's own date's rate from the cache, or leaves the pair rate-pending. There is no new surface: a rate-pending capture is counted by the existing Home/Trends F9 footnote above ("N entries pending rates", with its Check for rates action), exactly as a rate-pending fill-up is | None on the capture form - the pair saved as the user chose. The F9 footnote is where the pending row's next step lives (Check for rates · wait · edit the entry to add a manual rate) |
...
| Add receipt to an existing entry (PJ.48, RV.202, RV.204): photo could not be saved (disk full, the photo would not encode) | The entry SAVES anyway (never a blocked save, hard rule 1) and the shared "could not be kept" toast reports it after the entry is on disk – the SAME sentence the fill-up and expense saves show for the identical situation (RV.149). **One contract for every entry kind (RV.204)**: the fill-up edit, the service and the expense attach alike degrade, so the save the user asked for is never lost to a storage failure; the old fill-up-only "entry is unchanged" warn row is gone | Free up space and re-scan the receipt (the photo is gone from this save only; the entry stands) |
...
| An entry references a receipt photo that was never saved – no live `Attachment` row resolves to the id (RV.173's dangling id, RV.208) | The receipt strip shows **"The photo for this entry was never saved"** with "Add the receipt again to keep it with this entry." The reference is left in place, **never swept**: a row this device has not pulled yet is not locally distinguishable from one that was never written (docs/SYNC.md -> Attachments), so clearing it would delete a valid link for a user mid-restore. This is not the empty state – "Add receipt" alone would imply the entry never had a receipt and say nothing about the lost photo | **Add receipt** – the same camera/Photos door the empty card offers; the entry stays open and editable throughout (hard rule 1) |
...
| Entry lost to a sync merge (S1/S4) | "Overwritten by sync" section, read from the real `syncOverwrite` undo log (`RecentlyDeletedSyncOverwrites`): the entry's LOSING version ("Neste · 51.1 L · 84.77 €") with its own countdown - "Replaced Aug 21 · 28 days left · changed on iPad" (the device only when the wire attributed it) | Restore my version (writes the losing version back as a fresh local edit, S1/S4, hard rule 13; the log row is consumed) · leave it |
...
| Below data floor | Honest label: "first estimate · 1 fill cycle" / extended window "last 5 months". **RV.118 (2026-09-19): every headline carries its provenance line** - "last 3 months · 6 fills · 4 full tanks" on Home under the figure and as the Trends consumption tile's caption (the honest span label leads it, so an extended window still reads "last 5 months"; Home's first-estimate line drops the span because the label sits on the line above); a car with fills but no closed segment reads "Not enough data yet · 2 fills · 1 full tank" beside the D4 hint - the count it has, never a computed average. Three whole localised count phrases (real plural rules) joined by a middle dot, never a count glued to a word | Keep logging; the line says what the number is made of |
| Fill pattern (RV.120, 2026-09-19) | Under the vitals on Home and under the tiles on Trends, the **Fill pattern** card: "Between fills 500 km · every 10 days", "Range left 625 km · on a 50 L tank", "This month, on pace ≈ 207 € · after 15 days". **Each row is omitted, never zeroed or dashed, when it cannot be derived**: the range needs a tank capacity the user's own fills corroborated (a catalog figure nobody filled past 80% of is a guess), the forecast needs day 7 and two fills this month and an exact total, the spacing needs two fills; under the consumption floor the whole card is absent | Keep logging full tanks - the range appears once a fill corroborates the tank; the forecast is a prediction on this month's pace, not a bill |
| Anomaly detected (J9) | Amber insight card with evidence chart | Act (creates reminder due next year, opens it for edit) · dismiss (one tap, suppresses the cause) |
...
| **Entries the server rejected structurally (a 422, `payload_schema_violation` et al., RV.284)** | Row: "N entries could not sync" with the caption "Update the app or edit them to retry" (amber icon, never a modal). The count is derived from the `rejected` sync bookkeeping, never stored | **Edit the entry (it re-pushes) or update the app.** A 422 is not unavailability: the same bytes are rejected forever, so the honest next step is a newer app or a newer server - never the user's data (hard rule 7). Each row's badge taps through to edit |
...
| Storage quota near/full (blob 429, `blob_quota_exceeded`) | Row: "Photo storage 95% full – older photos stay on this phone only." plus the card's next-step line: "Everything on this phone keeps working – new photos upload when space frees up." (**RV.70** replaced the old "Tankbook Pro" button - it pushed a blank screen). **The percent is the server's own**, carried by the 429's additive `quotaUsedPercent` member and read from the last sync outcome (RV.253); a percentless 429 surfaces as 100, because exceeded IS full | **Nothing to fix and nothing to buy (RV.70): no tier exists in v1 (P6.16, `docs/STORE.md` §5), so the card names the wait as its next step and carries no control** - keep using the app on this phone (hard rule 1), older photos stay on this phone only, nothing is lost, and new photo uploads resume when the account's space frees (a tombstoned record's 30-day orphan sweep, never a periodic reset). A button that pushes a screen which does not exist is the bug this row now forbids (hard rule 7). **The state clears on the next successful blob upload** (RV.253): the card reflects the last cycle's outcome, so a sweep that frees space retires it rather than leaving a stale "95% full" |
...
and the **total** - and an **expense** recognition offers its **amount**, its **currency**, its
**category** and the receipt's **date** (RV.200's field set, extended by RV.246 and RV.280).
Every offered field keeps a distinct, stable accessibility id (the old
...
| A cloud reading landed after save, and it differs or fills a blank | An item: "Receipt reading ready · Finished after you saved." with a **per-field comparison** - every field the receipt read that differs or fills a blank renders "you entered X · receipt Y", marked, with a tick. The two acts read differently: a blank field shows an en-dash on the left and carries the caption **"Fills the empty field"**; a differing one shows the typed value beside the receipt's in **amber** and carries no caption - the colour and the pair say "replaces" without repeating it on every row (product owner, 2026-09-11). VoiceOver hears the verb for both. The entry keeps its own badge (hard rule 8). | Tick the fields to take · **Update from the receipt** (takes the ticked fields only, disabled until one is ticked; the prominent filled action once a field is ticked - RV.64) · **Leave it as it is** (nothing changes; the prominent filled action only while nothing is ticked - RV.64) · **Use a different receipt** (routes to Edit entry, where the receipt lives; deliberately not "replace" - that verb belongs to the FIELDS, RV.64) |
| A late **service** reading landed after save (RV.201; PJ.302–303) | The same card, offering the service's own fields: the **Vendor**, each cloud line and the **Total**, each tickable and each with its own stable id (`inboxTick_vendor`, `inboxTick_lineItem_<cloud index>`, `inboxTick_total`). **A line is paired onto the user's own row** (by amount, then title - `LineMatcher` - never by position): the label names THAT row ("Row 2"), the "you entered" column shows that row's title and cost, and the receipt column the cloud's. A cloud line no row matches is labelled **"New line"**, shows an en-dash on the left and reads **"Fills the empty field"**; a row the cloud did not see is not listed and never deleted. When the cloud's lines do not sum to the total it read, the **Total** row carries the amber caption **"Doesn't add up – check the lines"** (`inboxReadingDoesNotAddUp`); the ticks stay available - attention, not a block (hard rules 5, 7) | Tick the fields to take · **Update from the receipt** (writes the ticked fields back to the `ServiceRecord` - a ticked paired line rewrites its row, a ticked new line is appended) · **Leave it as it is** (nothing changes) |
| A late **expense** reading landed after save (RV.201) | The same card, offering the **Total**, the **Currency** it was read in, the **Category** it was read as and the receipt's **Date** (RV.200, extended by RV.246 and RV.280) - a differing amount, currency, category or date is offered, never applied on its own. A foreign total renders under its own symbol, never the entry's (hard rule 3). | Tick the fields to take · **Update from the receipt** (writes the `Expense`) · **Leave it as it is** (nothing changes) |
| A fuel reading whose own numbers cannot coexist (RV.288: `0.56 L x 1.954` beside a total of `0.00`) | The card keeps the reading but its litres, price and total are **not offered** - three numbers that fail their own cross-check are one bad read, not three corrections - and a caption in `inkSoft` says so: "The reading's numbers don't add up – its litres, price and total are left out. Check the photo before taking anything." A zero read is a field the model could not read, never an offer. The non-numeric fields (date, fuel kind, currency) are still offered where they differ; if none does, the card is the nothing-to-change one below | Check the photo · tick a non-numeric field · **Leave it as it is** · **Use a different receipt** |
...
| Ambiguous currency (the units half is N/A for v1 - both importers are metric, `units` reserved; `JOURNEYS.md` F6) | One question, once per file: "Which currency are these amounts in?" (ambiguous **dates** have their own row below, PJ.10) | Answer; import proceeds |
...
| **Preview before commit** (not an error - the gate) | "Here's what we read: 220 fill-ups · Mar 2023 - Aug 2026 · 118 930 km · EUR · **8.2 L/100km**". Figures the user can check from memory, F7's "numbers, not a checkmark" (`JOURNEYS.md` F6a). Names the target car, and the S2 duplicate count when merging. **RV.185:** when the import will create a NEW car, its name is an editable field pre-filled with the derived suggestion (the file's vehicle name, else the format name) - a derived value is editable when offered, not only later (hard rule 13). An existing destination car is never renamed or re-homed here | Import · edit the new car's name · adjust currency/car (units is N/A for v1) · fix flagged rows · cancel |
...
| **A row whose implied consumption is an outlier** (CHECK 5, RV.218; `JOURNEYS.md` F2) | The review list badges the fill **"Unusual consumption"** and quotes the engine's own figure ("This fill implies 1.6 L/100km – check the litres or the odometer."). The odometer and date are internally consistent, so **no single field is marked**: the two checks name the fields that could be wrong, and the row stays committable (hard rule 13). Distinct from the timeline row above, which names the odometer/date - the litres may be the misread field (RV.229) | Check litres · check odometer · import as-is · leave out |
