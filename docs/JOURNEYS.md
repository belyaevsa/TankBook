...
producer). **The cloud half is the invoice's own (PJ.29a, filed 2026-09-13).** The expense cloud half
shipped in **[PJ.29]** - an Expense-mode scan now calls `/extract` with `kind: "expense"`, and a late
cloud answer reaches the same inbox - but the service scan still splits its lines locally and never
calls the gateway, so carrying an invoice's header through `/extract` is its own row. **The captured
...
- **The gateway is not fuel-only (PJ.29, 2026-09-13).** An Expense-mode scan sends `kind: "expense"` and a within-budget answer fills the expense form's blank untouched amount, currency, date and category; a late answer reaches the same inbox through the ONE policy. So a shop or parking receipt is no longer recognised on-device alone. The service invoice's cloud half is filed as `PJ.29a` and does not ship in PJ.29.
