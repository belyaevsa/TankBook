...
| Verify | Sees preview: N entries, date range, detected currency/units, per-field mapping | Checking their known numbers | → Show *their* lifetime average consumption on the preview so they can check it against the number they remember from the old app (F6a - no importer's file carries the old app's own figure); a matching number = instant trust |
| Commit | Confirms; garage now shows full history and trends from day one | Relief, sunk cost transferred | ⚠ Silent unit/currency misread poisons all trends → flag ambiguous rows for review instead of guessing. **RV.185:** the currency the user declared becomes the new car's own home currency, so a KZT import does not land as a log of rate-pending rows against a EUR home nobody chose. **RV.255:** when the import **creates** a car - the single new target, or the first new lane of the multi-car gate - that car is selected, so the Home the wizard returns to shows the imported history instead of the user's old car; an import into an **existing** car leaves the user's chosen car selected |
...
producer). **The cloud half is the invoice's own and now ships (PJ.29a, 2026-09-13).** A service
scan asks `/extract` with `kind: "invoice"` as soon as the local split has produced its outcome -
the form is already open on it (F4). The image sent is the **first page** the document camera
captured: an invoice may have several, and the header is on the first. The provider is asked for the
header only (`vendor, total, date, currency`), and the **line items stay the local deterministic
split's** - a cloud answer never replaces them (J7's split is not a model's). The expense cloud half
shipped in **[PJ.29]** - an Expense-mode scan calls `/extract` with `kind: "expense"` - and both
kinds run under the same guards: the car's home currency and the locale as hints, the same
rendition, `allowsServerBacked` withholds the call under `.required`, a guest gets no transport, and
the 3 s budget bounds the user's wait, never the work. A within-budget answer fills only the service
form's **blank AND untouched** vendor, date and currency, as a suggestion (hard rule 13); the total
has no standalone field on the form (the header total is derived from the line items), so it is
offered only through the inbox. A late answer - or one that arrives after the save - becomes the
inbox item above through the ONE policy, never a value that moves under the user's cursor.
`ServiceRecognitionBuilder.reading(fromGateway:)` (core) builds the header pre-fill and the inbox's
recognition from one decode, so the two routes cannot disagree about what the invoice's header said.
**The captured
pages are persisted when the scan starts (RV.243, 2026-09-12):** a record saved before the read
...
**The cloud half ships for the expense kind (PJ.29, 2026-09-13).** An Expense-mode scan now asks
`/extract` with `kind: "expense"` - a shop, parking or toll receipt is a document kind of its own, so
the provider is asked for `total, date, currency, vendor, category` and never the fuel fields it does
not have. The request runs under the same guards the fill-up Confirm sheet uses: the car's home
currency and the locale as hints, the same rendition, `allowsServerBacked` withholds it under
`.required`, a guest gets no transport, and the 3 s budget bounds the user's wait, never the work. A
within-budget answer fills only the expense form's **blank AND untouched** amount, currency, date and
category, as a suggestion (hard rule 13); a late answer - or one that arrives after the save -
becomes the inbox item above through the ONE policy, never a value that moves under the user's cursor.
`ExpensePrefillBuilder.reading(fromGateway:)` (core) builds the form's pre-fill and the inbox's
recognition from one decode, so the two routes cannot disagree about what the receipt said, and an
unknown category string is dropped, never guessed.
...
*(Status cleared 2026-09-19: RV.288 added the late-answer stage below; the 2026-09-12c review predates it.)*
...
- **The gateway is not fuel-only (PJ.29, PJ.29a, 2026-09-13).** An Expense-mode scan sends `kind: "expense"` and a within-budget answer fills the expense form's blank untouched amount, currency, date and category; a service scan sends `kind: "invoice"` and a within-budget answer fills the service header's blank untouched vendor, date and currency (the line items stay the local split's). A late answer from either kind reaches the same inbox through the ONE policy. So a shop or parking receipt and a service invoice are no longer recognised on-device alone.
...
**Status: implemented 2026-09-12** (reviewed by REVIEW-SCENARIO, REVIEW-SCENARIO-F9-2026-09-12)
