...
`capture.classify` (PU.29) – one frame's pump-display classification, emitted from the capture pipeline before the receipt or pump arm runs: `reader` (`loaded` / `missing` – the bundle's models were or were not found; `missing` means the pump path is dead and every frame is a receipt), `display` (the verdict), `rows` (verified digit rows), `textLines` (Vision text lines), `widestRow` / `tallestRow` (the largest row as a fraction of the frame) and `durationMs`. It answers the question the `capture.pipeline` line cannot – *why did this photo run the receipt arm* – with counts and fractions only: the frame, its rows and any digit read have no route into the line (hard rule 12).
