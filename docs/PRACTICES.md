...
| S2 | **Tokens in the Keychain with the right accessibility class**; the database and attachments under file protection, *including* SQLite `-wal`/`-shm` | The WAL file holds the last N transactions in clear and is the file people forget | `SECURITY.md` -> iOS table; enforced by `SecurityInvariantsTests` (check in section 7) |
...
|---|---|---|---|---|---|
| `ios/` (nowhere) | URLSession default **60 s** connect/resource | Every API call in the app | absent | C, one named pair (connect ~8 s, read ~30 s) in `TankbookHTTPClient` | A half-connected radio freezes a sign-in, a sync tap or a gateway call for a full minute with no error (U6). **Bug** |
| `backend/src` (nowhere) | HttpClient default **100 s** | Rate feeds, JWKS, APNs | absent | C per named client | A slow ECB/CBR feed pins a job thread; a slow JWKS fetch stalls every sign-in behind it (U6). **Gap** |
...
| U6 | **MISSING** | No `timeoutInterval` anywhere in `ios/` - `URLSession.shared` with its 60 s default carries sign-in, sync, blob PUT and import multipart (`Auth/URLSessionTransport.swift:21`); no `Timeout` on any backend `HttpClient` - PR.6 |
...
| S2 | PARTIAL | Keychain classes correct and tested (`Auth/KeychainSessionStore.swift:30,95`). **No file-protection attribute is set on the database, `-wal`, `-shm` or any attachment file**; the promised file-protection test does not exist; `App/VehiclePhotoStore.swift:6-7` claims backup exclusion nothing implements - PR.16 |
...
| S4 | MET with a hole | Both redactors are single enforcement points and tested. **Forty view sites log `error.localizedDescription` with `privacy: .public` through a raw `os.Logger`**, outside the redactor and under a subsystem the diagnostics export does not read; no `TankbookLog` is constructed in the app target at all. Safe today only because GRDB's `publicStatementArguments` defaults to false - PR.5 |
...
| D2 | PARTIAL | Typed fields and redactors on both tiers. The app target bypasses the facade entirely (see S4) and no lint forbids `Logger(subsystem:` outside `Logging/` - PR.5 |
