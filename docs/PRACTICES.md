...
| `Service/ReminderOffer.swift` | 15 000 km / 12 months (oil), 12 months (insurance) | The curated per-category interval the post-save "remind you next time?" offer pre-fills (RV.77) | a | C | Correct: the table defines the *meaning* of each category's cadence (why each number: `SCHEMA.md` -> Reminder), and the offer makes every value user-editable in the same breath (hard rule 13) - so the default is a suggestion, never a fact |
