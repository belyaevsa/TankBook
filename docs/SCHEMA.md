...
- **What the service can actually serve for old dates is bounded by its feeds, and that bound is the honest limit of the drain** (RV.88, 2026-09-06). The ECB feed is *daily-only* (its XML carries today's date; RV.50), so an ECB-sourced pair dated before the service's first daily run has no row and **none can ever be created** – the answered mechanism (`rate_backfill_answered`) stops it being re-asked forever, and the client's demand fetch for it answers empty. The CIS feeds (`date_req`/`fdate`) serve past dates, so a RUB/KZT/AMD/GEL/BYN row of any age within a feed's archive **can** drain. A row that cannot drain stays rate-pending and is counted: it shows its original amount, is excluded from home-currency totals, and its next step is the entry's conversion card (a manual rate, hard rule 13) – never a silent zero, never a same-currency lie (docs/ERRORS.md -> Home).
