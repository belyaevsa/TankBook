...
1. **Three navigation kinds, three gestures:**
   ...
   - **Sheets** (Confirm variants, Car switcher, Tank level, Reminder complete, Sign in) – drag handle, swipe-down to dismiss, plus an explicit close/"Not now". A sheet with unsaved *typed* input asks before discarding ("Keep editing / Discard"); a sheet with only scanned data discards silently – the photo is never lost, it re-offers from the camera roll. **A decision that must be answered is the one exception (RV.177/RV.178)**: the home-currency question (Vehicle detail's Save, when the currency changed on a car with a log) is a custom sheet with no drag handle, no close and `.interactiveDismissDisabled(true)` – abandoning it would silently drop the pending save. It offers no cancel either: Save already committed the currency change, so both answers (a filled Convert, a quiet Keep) commit. It is a decision over an already-committed action, not a screen, so it is not a per-screen index row.
...
flowchart TD
    subgraph Onboarding
        ...
        Restoring -->|interrupted: Open my garage - partial, keeps filling| Home
        Restoring -->|interrupted: Retry now| Restoring
    ...
    Home -->|reminder chip| RemindersAll
    Home -->|All reminders chip| RemindersAll
...
| Restoring | successful sign-in with data | Open my garage → Home · **the interrupted state (PJ.39: the pull dropped mid-way - `RestoreInterruptedView`)** shows what landed so far and carries "Open my garage (partial, keeps filling)" → Home (absent when nothing landed) and "Retry now" (re-runs the pull from the persisted cursor); the regular sync resumes the rest once online · **the failure states (empty account, backend down) carry two doors**: "Import a file you exported yourself" → RestoreFromBackup (the LOCAL Tankbook-backup door, no network, no account - RV.260) and "Import from another app" → ImportWizard (the third-party MFM/Drivvo parser, which cannot read a Tankbook archive) | Cancel = sign out → Welcome (never traps) |
...
| Reminders | VehicleDetail (a car's own list); Home's chips land on the merged list | complete → ReminderComplete · New reminder → form · **the foot of the list carries the History section** (RV.248: terminal `.done`/`.dismissed` rows, each with its dismissal reason or the entry its completion logged, tappable to that entry) | back → opener |
