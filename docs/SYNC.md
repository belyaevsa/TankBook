...
**The server applies one push batch in ONE transaction (RV.105, 2026-09-07).** RV.97's count bound of 200 records only helps if the server's cost per record is sub-second; on the production host each record's commit (a begin → `SELECT … FOR UPDATE` → SCN allocation → write → **commit** cycle) cost a fixed ~578 ms – linear in record count, independent of payload size and CPU, because the commit's fsync is the price. 200 records of *small* changes therefore took ~115 s of pure commit time against the same 120 s upload budget the RV.97 bounds exist to fit, and a slower disk day turns that near-miss into a repeat of the livelock above at a bigger number. Since RV.105 `SyncRepository.ApplyBatchAsync` runs the **whole validated batch in one transaction**: each change is resolved against its row (`FOR UPDATE`), accepted changes are written with an SCN allocated inside the same transaction, and the transaction **commits once**. The measured production cost of a push therefore drops from ~578 ms/record to ~1 commit per batch. What partial acceptance now means mechanically: a conflict or an idempotent replay is a **no-op inside the transaction** – the row is read and left untouched, the batch continues, and the conflict is reported per-item – never a rollback, so one conflicting record still cannot take the other 91 down. The SCN invariants survive unchanged because allocation and commit stay inside one transaction: the `account_seq` row lock is held from the first allocation until the single commit, so the batch's SCNs are contiguous and become visible in commit order, and a pull cursor paging the stream during concurrent pushes still never skips a record (`Pull_UnderConcurrentWrites_NeverSkipsARecord` owns this). The `sync.push` log event carries a **`Commits` count** so a per-record-transaction regression shows in the log even on a host whose fsync is too cheap for `DurationMs` alone to expose it (LOGGING.md §3).
...
- **A `Vehicle` field merge reports whether it changed anything** (RV.14, RV.136). `mergeVehicle` compares the merged result against the two inputs at the **decoded** level – the `Vehicle`, never raw payload bytes, which do not converge across a lossy re-encode (key ordering, decimal formatting, server normalisation). It returns `.remote` when the merged content equals the remote (nothing local to push → applied as synced at the remote's SCN), `.local` when it equals the local (already correct → the SCN is recorded, no dirty), and `.fieldMerge` only when the merged content is a genuinely new combination of both sides (→ dirty, push). **The per-field version map is deliberately not part of that judgement.** A map carries a whole-record fallback (a missing stamp reads as that side's `clientUpdatedAt`), so merging two maps of different coverage – one side stamped per-field, the other not – yields an element-wise max that is a THIRD map no side holds even when the decoded content is identical. The RV.14 form of this bullet also demanded the merged map equal one side's map, so that phantom was reported as `.fieldMerge`, the unchanged row was re-stored `.dirty`, and the pull manufactured a push of content the server already had – the echo loop that recurred on a later build (RV.136). Content equality is the truth: if the merge changed no field value there is nothing to push, whichever map the bookkeeping suggests.
- **Record-level LWW's `.local` arm compares decoded values, not bytes** (RV.35). When a synced record wins LWW against a pulled copy (clock skew), `applyPull` re-dirties it only if the record actually differs: `RecordMerge.recordsEqual` decodes both sides to the typed entity and compares those, falling back to payload bytes only when the type cannot be decoded. A lossy round-trip – a normalised number token (`1.0` vs `1`), a decimal string with a dropped trailing zero (`289.50` vs `289.5`), a date re-serialised without fractional seconds – changes the bytes without changing the record, and re-dirtying on those bytes pushed the record back forever (the preferences echo, 2026-09-03). A genuine decoded divergence, or a differing `deleted` flag, still re-dirties and pushes, so nothing is lost silently (hard rule 8). `recordsEqual` carries **one case per synced entity type** – `Vehicle` included, so a tombstoned car cannot fall to the byte comparison either – and the byte fallback is reachable only by a type this build does not know; a test walks the closed catalog and fails when a synced type has no case, so a new entity cannot inherit the byte-compare echo.
...
- **The pull cursor is per account and monotonic (RV.249), and its advance is durable at the pull that earned it, not at the end of the cycle (RV.155).** `SyncEngine.pullAll` persists `nextSince` as soon as the page is applied. A restore seeds its cursor at 0 rather than buffering advances in memory: it still pulls the whole account from 0, but every page it applies is written through to the device store immediately. That matters because a restore and the app's regular sync are not serialized – the restore runs the engine directly, while the regular sync runs on the coordinator's in-flight gate – so a slow push on one cycle must not leave the other reading a stale cursor and re-downloading the delta. A failed pull advances nothing. The durable store is keyed by account id (`tankbook.sync.cursor.<accountId>`) because sign-out clears the session but never the cursor: an unkeyed cursor would let a later sign-in to a different account resume from the previous account's SCN and skip its history, and a monotonic guard alone would make that permanent. `save` ignores a lower advance for the same account – an in-flight restore page returning a lower `nextSince` than the app engine already persisted is a stale replay, and SCN only moves forward for a given account. A pre-RV.249 unkeyed value is migrated into the signed-in account's slot on first load and the old key deleted, so an existing device keeps its place instead of replaying the account once.
...
- **The rate cache itself never syncs** – each device fetches its own feed; only `Money` snapshots inside entries travel. A device with no feed access simply leaves entries pending; month totals and the log's dividers never sum a pending row as zero – they report the known figure as `.partial`/`.pending` with the F9 footnote (RV.106) until any device fills them. The footnote's "Check for rates" runs a demand drain over the pending rows' own dates (`MoneyBackfillService.demandDrain`, RV.111) – never the rolling pack, which cannot reach a row dated years back – and a reached demand that leaves such a row pending is a dead end the footnote names by pointing at the manual rate (same silent fill, RV.106/RV.111).
- **The demand drain is user-initiated, and it reports its outcome (RV.132).** The automatic pass is silent; the one rate door that may speak is the user's own "Check for rates" tap: the footnote acknowledges it immediately ("Checking for rates…") and a filled or nothing-pending drain posts a toast, while the dead end stays the footnote's standing copy and an offline pass stays a non-event. The drain is never deferred by Low Power Mode (it passes `.userInitiated`); the automatic pass still defers silently while the mode is on.
