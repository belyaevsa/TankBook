...
| **[ ]** P0.12c | `apiBaseUrl` guardrails: compiled-in host allowlist, health-gate before adoption, auto-revert on N consecutive failures, and the HTTP client's request-time allowlist + host-bound `Authorization` | L1 **brick-proof**: unreachable `apiBaseUrl` + N failures auto-reverts to bundled and recovers. L1: non-allowlisted host rejected by the HTTP client **and no `Authorization` header ever constructed for it**. L1: allowlist near-miss (`evil-tankbook.app`, `tankbook.app.evil.com`) rejected against a `tankbook.app` suffix rule. L1: failing candidate URL discarded, previous value stands |
