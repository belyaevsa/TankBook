...
| ID | Task | Checks |
|---|---|---|
| **[x]** P2.1 | Capture camera screen: modes, auto-detect surface, permission fallback. **NOT reachable in a Release build** - nothing routes to it outside the DEBUG `-presentScreen` hook, and the artboard's centre capture button was never built. Tracked below | L4: F8 denied-permission flow (manual form + deep link); mode row snapshot; "Type it" path |
...
| P2.1b | **Wire Capture into the app.** The artboard's centre capture button in the tab bar was never built, and every `onCapture` callback opens the manual form instead, so `ModalRoute.capture` is set only by the `#if DEBUG` launch hook - the screen is dead code in Release. This also means the app currently offers one door, not two (hard rule 15) | L4: a UI test reaches Capture **without any launch argument** - that assertion is what stops this recurring; centre button matches `design/screens/*.dc.html`; `SCREENMAP.md` line "Home -->\|capture button\| Capture" becomes true |
