...
**Decided 2026-08-30 (product owner): the QR is never named in user-facing copy, feature lists or positioning – not on Welcome, not on Capture, not on the site or the store page. It is part of the receipt.** So the QR is **not a capture path and not a differentiator on its own**. It is an **anchor that makes OCR trustworthy**, which is worth more than it sounds: picking the wrong total is the parser's worst failure mode, and on the corpus the QR's `s` fixes three of its four total errors outright (it had read the VAT line twice and a rounding line once). On the fourth – a mixed fuel-and-water receipt – the QR's grand total *disagreeing* with the extracted fuel line is precisely how mixedness is detected, which is what hard rule 4 needs. It also gives an exact date and, via `fn+i+fp`, a duplicate key.
...
> Logging a fill-up should take seconds, through whichever door you choose - type it or scan it. Your car's entire financial history lives on your phone, readable by you and no one else.

- **Two doors, always: type it or scan it.** Manual entry and capture are **peer paths of equal standing**, offered side by side at every entry point - never "scan, and type only if scanning failed" (hard rule 15). The corpus is what forced this, and re-measuring it on 2026-09-05 did not change the conclusion: receipts resolve **188 of 220 asserted cells (85%)** but only **33 of 48 receipts (69%) come out with every cell right**, pump displays commit **24 of 178 numeric cells (13%)** and ship off, Vision still misreads a digit at **confidence 1.00**, and a fiscal QR is present on **22 of 48** real receipts and carries just 2 of the 5 fields. A capture is therefore a **head start, not an answer** - whatever it produces is a default input the user edits (hard rule 13), so a poor scan degrades to "correct two fields", never to "start over". A camera-first design would punish the user every time the camera cannot deliver, and the numbers above say that is often.
...
5. **Fallback** – cloud multimodal LLM, ~~opt-in, Pro feature~~ **free tier, 50 reads/day** (RV.4, 2026-09-03: Pro is cut from v1, so gating it behind Pro gated it behind nobody while the served config promised 50). **And it is no longer a "fallback" in the timing sense**: measured in production it takes **12-36 s** against a 3 s UI budget (RV.51), so it essentially never arrives while the user is still on the sheet. The local read opens the entry immediately and the cloud answer lands in the **inbox** afterwards - the inbox is the normal path, not the exception

**Decision (amended 2026-08-25):** Apple-first hybrid, now **without** the Foundation Models step. On-device Vision plus the deterministic rules parser carry the primary path at zero marginal cost and full privacy; step 4 was cut because the on-device LLM has no Russian. The cloud LLM fallback handles crumpled thermal paper and glare-heavy pump displays – clearly labeled, and **free at 50 reads/day** rather than paid (RV.4). **Re-measured 2026-09-05, the primary path is much stronger than when this was written**: the rules parser resolves 188 of 220 asserted receipt cells (85%), with 33 of 48 receipts entirely correct, against the frozen cloud arm's 84/96 - so the gap the cloud exists to cover has narrowed, while its 12-36 s latency has not. The arithmetic cross-check gives a built-in confidence signal that competitors' "inaccurate calculations" complaints show they lack.
...
| Receipt scan | MVP | The hero feature. On-device pipeline above; user confirms pre-filled card. On RU/KZ receipts the fiscal QR printed on the slip, when present, anchors the total and date inside the same scan – an internal accuracy aid, **never a feature we name** (decided 2026-08-30, product owner: the QR is part of the receipt, so "we read QR codes" is not a capability, and it carries 2 of 5 fields on 22 of 48 corpus receipts). |
| Pump display photo | MVP **(on, 2026-09-26)** | For stations without receipts. Reads liters / price / total off the pump. **Turned on by the product owner on 2026-09-26** once the trained reader cleared the gate (P2.7): on the 68 frozen heldout photographs it commits 119 of 183 numeric cells, 118 correct (precision 0.992 against 0.99, coverage 0.65 against a 0.60 floor). A seven-segment display drops its decimal point per field, and a factor-of-ten volume is invisible to the arithmetic cross-check - which is why the reader measures each currency's decimal conventions and refuses a field rather than guess. Dark LCDs behind glass and TFT screens are not read yet (PU.90-PU.92); on them the reader refuses. Measured, not assumed: `docs/EXTRACTION.md` -> "The pump reader". |
