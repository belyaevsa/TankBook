...
| Pump display photo | MVP **(built, ships OFF)** | For stations without receipts. Reads liters / price / total off the pump. **The flag is off and the gate is what keeps it off** (P2.7): the reader commits 24 of 178 numeric cells across 66 pump photographs - every committed value correct, but 13% coverage against a 60% floor. A seven-segment display drops its decimal point per field, and a factor-of-ten volume is invisible to the arithmetic cross-check and permanent in the user's history, so the mode abstains rather than guess. Measured, not assumed: `Spike/ReceiptSpike/fixtures/high-water.json`. |
