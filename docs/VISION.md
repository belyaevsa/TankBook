...
| Receipt scan | MVP | The hero feature. On-device pipeline above; user confirms pre-filled card. On RU/KZ receipts the fiscal QR printed on the slip, when present, anchors the total and date inside the same scan – an internal accuracy aid, **never a feature we name** (decided 2026-08-30, product owner: the QR is part of the receipt, so "we read QR codes" is not a capability, and it carries 2 of 5 fields on 9 of 16 corpus receipts). |
