...
final class SignInFlow {
    enum Phase: Equatable {
        ...
        /// The pull dropped mid-way; the snapshot is what landed so far (nil
        /// when nothing did). Retry now, or open the partial garage - the
        /// regular sync resumes from the persisted cursor once online.
        case restoreInterrupted(RestoreSnapshot?)
    ...
    func applyScenarioIfRequested(_ arguments: [String] = ProcessInfo.processInfo.arguments) {
        ...
        // prefix of the others, and "-signInRestoreInterrupted" of its Empty.
        if arguments.contains("-signInRestoreEmpty") {
            phase = .emptyRestore
        } else if arguments.contains("-signInRestoreUnreachable") {
            phase = .restoreUnreachable
        } else if arguments.contains("-signInRestoreInterruptedEmpty") {
            phase = .restoreInterrupted(nil)
        } else if arguments.contains("-signInRestoreInterrupted") {
            phase = .restoreInterrupted(SignInTestSeed.restoreSnapshot())
    ...
    private func performRestore(accountId: String) async {
        ...
        case .unreachable:
            ...
            phase = .restoreUnreachable
        case .interrupted(let stats):
            // The pull dropped: what landed is shown and usable, the persisted
            // cursor makes the next pull resume (the regular sync's foreground
            // pass and PR.7's retry, or "Retry now" here).
            phase = .restoreInterrupted(stats.map {
                RestoreSnapshot(stats: $0, email: signedInEmail, provider: signedInProvider ?? .apple)
            })
        case .deviceRevoked:
            signOutLocally()
