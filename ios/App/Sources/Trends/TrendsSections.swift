...
enum TrendsFormat {
    ...
    /// The arrow follows the DISPLAYED figure (RV.296): improving consumption
    /// is a falling L/100 and a rising MPG, so the glyph flips for an inverted
    /// unit while the direction's meaning - and the colour - does not.
    static func headlineDelta(_ change: HeadlineChange?, unit: HeadlineUnit) -> String? {
        guard let change else { return nil }
        let figureRises = (change.direction == .improving) == ConsumptionDisplay.isInverted(unit)
        return String(format: "%@%.0f%%", figureRises ? "▲" : "▼", change.displayedPercent(in: unit))
    }

    static func headlineDeltaSpoken(_ change: HeadlineChange?, unit: HeadlineUnit) -> String? {
        guard let change else { return nil }
        let percent = String(format: "%.0f", change.displayedPercent(in: unit))
        let figureRises = (change.direction == .improving) == ConsumptionDisplay.isInverted(unit)
        let format = L10n.localize(figureRises
    ...
    /// when a second price exists and the move clears the shared noise floor
    /// (RV.297 - an unchanged price is not a rise), else the last fill's date -
    /// a caption, never a fabricated trend.
    static func priceCaption(series: [TrendPoint]) -> String? {
        guard let last = series.last else { return nil }
        guard let previous = series.dropLast().last, previous.value > 0,
              abs(last.value - previous.value) / previous.value >= TrendDirection.noiseThreshold else {
