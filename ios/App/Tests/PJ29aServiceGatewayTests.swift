...
/// PJ.29a - the service invoice's cloud half. These drive the REAL session
/// (`ServiceInvoiceSession`) with a recording transport, so the request the
/// shipped path builds is asserted directly - `kind: "invoice"`, the hints, and
/// the late-answer boundary - without a paid call. The full capture -> save ->
/// inbox path is the L4 `GatewayCaptureUITests`.
///
/// The named vacuous trap this guards: sending `kind: "receipt"` for an invoice
/// (the fuel prompt answers with volume and fuel kind, the mapping drops them,
/// and the cloud never learned it was a service invoice). The first test fails
/// on exactly that mutation.
@MainActor
final class PJ29aServiceGatewayTests: XCTestCase {
    ...
    private func answer() -> GatewayExtraction {
        GatewayExtraction(
            total: .init(value: Decimal(string: "148.00")!, confidence: 0.9),
            date: .init(value: "17.08.2026", confidence: 0.8),
            currency: .init(value: .eur, confidence: 0.9),
            vendor: .init(value: "Cloud Garage", confidence: 0.9),
            pipeline: "test")
    }
    ...
    private func makeVehicle() throws -> Vehicle {
        ...
        let vehicle = Vehicle(
            ...
            name: "PJ29a Volvo", make: "Volvo", model: "V60", year: 2015,
    ...
    private func savedService(vendor: String = "Local Garage") throws -> ServiceRecord {
        let vehicle = try makeVehicle()
        let now = Date()
        let service = ServiceRecord(
            id: UUID.v7(), createdAt: now, updatedAt: now, deletedAt: nil,
            vehicleId: vehicle.id, date: now, odometer: nil,
            money: Money(amount: Decimal(string: "89.00")!, currency: .eur, homeCurrency: .eur),
    ...
    func testTheRequestCarriesTheInvoiceKindAndTheHints() async throws {
        ...
        XCTAssertEqual(request.kind, "invoice",
                       "the service capture must ask for the invoice kind, never 'receipt'")
    ...
    func testALateInvoiceAnswerIsRoutedToTheInboxThroughTheOnePolicy() async throws {
        ...
        session.startGateway(
            ...
            onSavedAnswer: { extraction, entryID in
                XCTAssertEqual(entryID, service.id)
                inbox.recordLateGatewayAnswer(
                    .service(ServiceRecognitionBuilder.reading(fromGateway: extraction).recognition),
                    entryID: entryID)
            })
        await Task.yield()
        ...
        XCTAssertTrue(inbox.hasItem(for: service.id),
                      "a cloud invoice answer that lands after the save must reach the inbox")
    }

    func testTheKindIsTheInvoiceKindEvenWhenTheAnswerIsWithinBudget() async throws {
        ...
        XCTAssertEqual(pending.vendor?.value, "Cloud Garage")
        XCTAssertEqual(pending.total?.value, Decimal(string: "148.00"))
